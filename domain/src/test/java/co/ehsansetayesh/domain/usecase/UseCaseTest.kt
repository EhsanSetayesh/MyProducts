...
class UseCaseTest {
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun testExecuteProductException() {
        useCase = object : UseCase<UseCase.Request, UseCase.Response>(configuration) {
            override fun process(request: Request): Flow<Response> {
                assertEquals(this@UseCaseTest.request, request)
                return flow {
                    throw UseCaseException.ProductException(Throwable())
                }
            }
        ...
        runBlockingTest {
            val result = useCase.execute(request).first()
            assertTrue((result as Result.Error).exception is UseCaseException.ProductException)
        }
    }
